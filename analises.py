...
def mostrar_comparativo_ano(df_filtrado, cidades):
    ...
    cvli_pivot = cvli_por_ano.reset_index().sort_values(by="CIDADE FATO")
...
def mostrar_comparativo_mes(df_filtrado, cidades, anos, meses):
    ...
    cvli_mes_pivot = cvli_mes.pivot(index="CIDADE FATO", columns="Mes_Ano", values="Total").fillna(0).astype(int).reset_index()
